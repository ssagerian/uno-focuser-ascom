    ...
    internal sealed class UnoFocuserTransport : IDisposable
    {
        ...
        public void Connect()
        {
            lock (syncRoot)
            {
                ...
                // Some USB/serial interfaces can produce a spurious first command
                // immediately after the port is opened, so allow a few attempts.
                string id = "";
                ...
                if (!identified)
                {
                    ...
                    throw new InvalidOperationException(
                        $"Unable to identify UNO focuser. Last response: '{id}'");
                }
            }
        }
